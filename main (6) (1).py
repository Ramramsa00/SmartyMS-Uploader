...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "Checking status Okay... Command is Private.🌚**Bot Made BY @Ramramsa00**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
    )
...
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Hi I am bahubly TXT Downloader📥 Bot.**\n🔹**Send me the TXT file and Just wait and Watch😚.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
    file_name, ext = os.path.splitext(os.path.basename(x))
    credit = f"@ramramsa00bot"
    ...
    await editable.edit("**Enter Your PW Token For 𝐌𝐏𝐃 𝐔𝐑𝐋  or send 'Ram' for use default🎀**")
    ...
    await m.reply_text("𝐀𝐋𝐋 𝐃𝐎𝐍𝐄 😎✅🔸🔹 Download By💳: @Ramramsa00")

# Advance

@bot.on_message(filters.command(["ram"]) )
async def txt_handler(bot: Client, m: Message):
    editable = await m.reply_text(f"**🔹Hi I am Bahubaly TXT Downloader📥 Bot.**\n🔹**Send me the TXT file and Just wait and Watch🥵.**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
    file_name, ext = os.path.splitext(os.path.basename(x))
